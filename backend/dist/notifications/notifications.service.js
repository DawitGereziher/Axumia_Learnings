...
let NotificationsService = NotificationsService_1 = class NotificationsService {
    ...
    wrapHtmlTemplate(title, bodyHtml) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    async sendSms(phone, message) {
        const token = this.config.get('AFROMESSAGE_TOKEN') || this.config.get('AFROMESSAGE_API_KEY');
        const senderId = this.config.get('AFROMESSAGE_SENDER_ID');
        if (!token) {
    ...
    async notifyInstructorNewBooking(instructorEmail, bookingId, studentName) {
        const html = `
    ...
    async notifyStudentBookingConfirmed(studentEmail, meetingLink, slotTime, studentPhone) {
        const html = `
    ...
    async notifySessionReminder(email, meetingLink, timeLabel, phone) {
        const html = `
    ...
    async notifyPaymentReceived(email, amount, currency, itemTitle, phone) {
        const html = `
    ...
    async notifyInstructorPayout(email, amount, currency, phone) {
        const html = `
    ...
    async notifyCoursePublished(instructorEmail, courseTitle) {
        const html = `
    ...
    async notifyBidAccepted(email, requestTitle) {
        const html = `
